...
class MainActivity : AppCompatActivity() {
    ...
    private fun applyColorToSelection(color: Int) {
        ...
        if (targetStart >= targetEnd && newText.isNotEmpty()) {
            if(start == end) defaultTextColor = color
            else return
        }
        ...
        etText.text = newText
        if(start != end) etText.setSelection(start, end)
    ...
    private fun renderTextImage(textToRender: CharSequence?) {
        ...
        val layout = createTextLayout(textToRender, width)
        
        finalCanvas.save()
        val xPos = (width - layout.width) / 2f
        val yPos = (height - layout.height) / 2f
        finalCanvas.translate(xPos, yPos)
        layout.draw(finalCanvas)
        finalCanvas.restore()

        imagePreview.setImageBitmap(finalBitmap)
    }

    private fun createTextLayout(text: CharSequence, width: Int): Layout {
